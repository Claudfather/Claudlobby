...
- **`claudlobby move-bot` command** — migrates a bot between fleets. Auto-detects source fleet, copies `.env` secrets and `memory/` files, stops and removes the old service (systemd/launchd), regenerates the target bot, updates Telegram `access.json` group chat, and re-enrolls via `spin-up-bot.sh`. Dry-run by default, `--apply` to execute. `--cleanup-source` removes the old bot dir. Pre-flight checks refuse to move bots with uncommitted WIP or missing target stanzas. 10 tests. Resolves [#298](https://github.com/Claudfather/Claudlobby/issues/298).
