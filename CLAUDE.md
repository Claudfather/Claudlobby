...
| `interactive-claude-spike.sh` | #729 stage-B mechanic proof — boots ONE real INTERACTIVE claude in a throwaway root (freshbox auth+trust seed on a fresh `CLAUDE_CONFIG_DIR`), dispatches via `lib/dispatch.sh`, awaits the `report-back.jsonl` ledger row, then recovers the session transcript from `CLAUDE_CONFIG_DIR/projects/` and attributes it via `transcript-usage.py`. The reusable primitive the A/B comms-eval harness (stage C) + freshbox need — no other script does interactive + real binary + fresh isolated config. Opt-in `SPIKE_REALBOOT=1` |
