...
| `dispatch-overdue.py` | Finds overdue dispatches — the matcher behind the `fleet-pulse.sh` watchdog (age-capped via `DISPATCH_OVERDUE_MAX_AGE_S`). Also owns the two #835 doors: `--open-task` (the id `report-back.sh` resolves when a worker omits `--task`, so an id-less report still closes its dispatch) and `--orphans` (past-deadline rows whose worker respawned after dispatch — inert for alerting because the session holding the id is gone, but listed, since work lost to a restart is actionable). `--open` (#904) is the read door's list form: every still-open id'd row, **deadline-blind**, so it is a strict superset of `--all`'s and "carrying three tasks, none late yet" becomes expressible — a state no other mode could say. `--open-task` is now literally its head rather than a second loop, because a resolver that could hand back an id the list does not contain is the same desync class `_terminal_reported_ids` exists to prevent. A row missing `expected_by` reports it as `-` and is still listed: filtering it out would hide work the resolver can still close. **The grammar splits and that split was silent** (#1187): `--open`/`--open-task` take the BOT first, `--all`/`--orphans`/`--unassigned` take the LOGS first, so passing one the other's order keeps the ARITY valid — a path lands in the bot slot, nothing matches, and it prints nothing at rc 0, which is byte-identical to a genuine "nothing open". That is how a manager read a full backlog as all-clear while checking whether its own closures had worked. Wrong *count* was never the hazard and still fails loudly at rc 2; the missing check was on SHAPE, so those two modes now refuse a first positional that cannot be a bot id (holds `/`, ends `.jsonl`, or is blank) — lexical only, never a roster read, because this module is a pure function of (dispatch log, report ledger, clock) and a manifest lookup would re-import the #526 cross-fleet mismatch into a validator that would then refuse a legitimate name. The shape gate only kills the *instance*: a typo or another fleet's live bot passes every shape test and still returns zero rows, so `--open` also **states its scope on every run** — coverage honesty applied to a read door, since an empty result that names what it filtered on cannot be read as nothing-exists. That line goes to **stderr, and the placement is measured, not stylistic**: `report-back.sh:117` pipes this stdout through `awk '{print $3}'`, so on stdout the scope line is parsed as a row whose field 3 is `->`. The bite is narrower than it looks and has to be run rather than argued — a bot *holding* an open row still matches its own id (the phantom adds an entry, it does not displace the real one), so that case reads clean and looks like proof the placement is free; it fires where the bot has **nothing** open, where a valid id meets an open set of exactly `["->"]` and a correct report is flagged `supplied-id-not-open`. `--open-task` gets the shape gate (shared grammar, shared hazard) but **not** the scope line: it is machine-consumed and prints one id or nothing. **The remaining gap is SINGLE-BOT MODE and only it** — and getting that list right matters more than it looks, because an earlier version of this row and of the module docstring filed single-bot mode under "logs first", which is backwards and makes the next fix look harder than it is (#1188 review). The real split is whether a mode names ONE bot at all: `--open`, `--open-task` and single-bot mode each take one FIRST — `main()` reads `bot, dlog, rlog = argv[1], argv[2], argv[3]` for single-bot, character-identical to the two gated doors — while `--all`/`--orphans`/`--unassigned` name none and report across every bot. So single-bot mode carries the hazard *exactly*, is reached most easily by FORGETTING a flag (`<dlog> <rlog> <now>` falls straight through to it), and closes with the same one-line `_reject_bot_slot` call rather than any dlog/rlog-swap detection. The other three need no gate at all: nothing to check, and mis-ordering them is already LOUD — a ledger path lands in the `now` slot and `int()` raises, rc 1, measured on all three. Single-bot mode is the only silent shape left. **`--orphans` is no longer one of them** (#1014): orphan-ness is a comparison against `<bots_dir>/<bot>/data/.spawn`, so with no readable bots dir the answer is UNKNOWN — yet the mode printed an empty set at rc 0, making *cannot look* byte-identical to *nothing was lost to a restart*. Measured: no `--bots-dir`, a real one with no orphans, and one naming a path that does not exist all returned 0 bytes at rc 0 against a 295-row log — THREE states, one output, and the collapsed one reads as good news. It now refuses at **rc 3**, deliberately not the usage code 2: the flag is optional in the grammar so this is not a malformed call, it is a question the run cannot answer, and a caller must be able to tell a typo from an unreachable instrument. `orphaned_all` and `_classify_all` are **unchanged** — the former still returns `{}` without a bots dir, because `brief.py` imports it directly and labels the gap its own way; the refusal lives in the CLI mode, which is the surface a human reads. Both shipped callers pass a real `--bots-dir` *and* use `|| true`, so it cannot fire for either: inert for the watchdog by construction, which is the intended blast radius. Arity is now checked BEFORE indexing in the `--all`/`--orphans` branch too — pre-existing and verified on main, `--orphans <dlog>` died on an uncaught `IndexError` at rc 1 instead of the usage line, found by the test asserting rc 3 and rc 2 are distinct |
...
| `plane-parity.py` | Dual-write parity: legacy JSONL ledger ↔ plane rows, the canary's verdict instrument and a permanent reconciliation door. Join contract doors must honor: `source_ref = "<ledger-name>:<legacy-id>"` on every emitted envelope — parity then needs no per-door schema. Unreachable ≠ empty: absent/unreadable ledger or db (or a db with no plane schema) REFUSES at rc 3, while an existing empty ledger is a door that has not fired yet (rc 0, says so); malformed and id-less rows counted + disclosed, never dropped; rc 1 = parity broken |
