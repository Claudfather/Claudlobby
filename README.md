...
- `library/` — 19 expertise profiles (manager, engineer, reviewer, designer, business, data-engineering, …), 53 skills (dispatch, lifecycle, prs, sweep, fleet-status, briefing, status, triage, …), 17 MCP fragments (github, gws, google-analytics, google-search-console, meta-ads, meta-business, posthog, notion, slack, shopify, printify, homeassistant, docker, spotify, granola), 24 guardrails, 39 protocols
- `lib/` — 78 bash lifecycle scripts: `start-bot.sh`, `keepalive.sh`, `report-back.sh`, `tg-post.sh`, `creds-check.sh` (daily credential keepalive), `fleet-state-update.sh`, and more
