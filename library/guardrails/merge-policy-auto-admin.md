...
**Why the rungs above carry the whole weight.** There *is* a server-side backstop — it simply does not cover this. Measured across six repos of one estate via the repository **rulesets** API: five carry a ruleset enforcing branch deletion, non-fast-forward and pull-request rules, three of those requiring an approval; the sixth is on a plan returning `403 Upgrade to GitHub Pro or make the repository public` and so cannot have one. **Not one of them declares `required_status_checks`. Zero of five.**
