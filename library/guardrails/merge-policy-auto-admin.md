...
Of the six that carry a ruleset, **four DECLARE an approval requirement and only three ENFORCE one** — declared and enforced are separate counts, which is what *A present ruleset proves nothing until you read its `enforcement` field* (below) is about. **These two counts measure different things and both appear above** — the *five* counts repos with no rule enforced at all (a repo can enforce branch-deletion while requiring zero approvals), while *three* counts repos actually requiring an approving review. Quote whichever you mean and say which. Read the rule, not its presence: one active repo carries a `pull_request` rule with **`required_approving_review_count: 0`**, which declares the *absence* of an approval requirement rather than one; and one of the disabled pair carries no `pull_request` rule at all.
