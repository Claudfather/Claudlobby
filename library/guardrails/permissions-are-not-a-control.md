...
- The **MCP-tool** deny (`deny: [mcp__github__merge_pull_request]`, the `monitor-read-only` pattern) is untested and **will stay untested**. The direct test is merging a real pull request, and that cost exceeds the value of confirming what the root cause already predicts. **Do not attempt it, and do not read this as a to-do.**
