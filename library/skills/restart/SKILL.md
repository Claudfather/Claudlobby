...
Flow: `/claudna:session-handoff --auto` -> notify on Telegram -> `launchctl kickstart -k gui/$(id -u)/<SERVICE_PREFIX>.<BOT_NAME>`

## Steps

1. **Run session handoff.** Invoke the `/claudna:session-handoff --auto` skill. This captures session context, validates memory, and writes the handoff file — all while the session is still alive and responsive.
...
3. **Restart.** Run `launchctl kickstart -k gui/$(id -u)/<SERVICE_PREFIX>.<BOT_NAME>`. This will:
   ...
   - The new session auto-runs /claudna:context-resume and notifies the user on Telegram
