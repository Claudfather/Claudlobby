...
**Mandatory worker post moments:** progress milestone (~2-3 min during active work), completion (+ PR link, tag manager), blocked (+ run `report-back.sh blocked`), unexpected scope change. (No per-task acknowledgement post — the ack is your first `[BOTREPORT]` row, per Worker Lifecycle Step 2; Telegram sees milestones and outcomes.)
