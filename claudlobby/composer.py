...
def compose_bot_conf(bot: BotConfig, fleet: FleetConfig, paths: Paths) -> str:
    ...
    lines = [
        ...
        f"BOT_SERVICE={_shq(f'{fleet.service_prefix}.{bot.bot_id}')}",
...
def _write_timer_units(
    ...
    """Write the .service/.timer/.plist units for a single fleet timer.

    One emitter shared by the system_defaults timer loop and the opt-in
    fleet.sweep branch (consolidate, don't fork).
    """
    ...
    service_lines = [
        "# Generated by claudlobby — do not hand-edit.",
        "[Unit]",
        f"Description=claudlobby {name} ({fleet.name})",
        "",
        "[Service]",
        f"Type={svc_type}",
        f"Environment=CLAUDLOBBY_ROOT={paths.root}",
        f"Environment=CLAUDLOBBY_FLEET={fleet.name}",
        f"ExecStart={script.replace('$CLAUDLOBBY_ROOT', str(paths.root))} {fleet.name}",
    ]
    ...
    if sched["type"] == "interval":
        secs = sched["seconds"]
        timer_lines.append(
            f"Description=claudlobby {name} timer ({fleet.name}) -- tick every {secs}s"
        )
        timer_lines.extend(
            [
                "",
        ...
        timer_lines.append(
            f"Description=claudlobby {name} timer ({fleet.name}) -- {expr}"
        )
        timer_lines.extend(
            [
                "",
...
def compose_fleet_timers(
    ...
    if emit_defaults:
        for name, cfg in timers.items():
            ...
            _write_timer_units(
                timers_dir, prefix, name, sched, script, svc_type, fleet, paths
            )
