...
"""
                ...
                # is global (AFTER the operator include, so these keys win).
                out += f"""
# App commit identity (F-identity variant a) — AFTER the operator include,
# so these keys win. The noreply address maps to the App bot user, so
# commits attribute to the App, not the operator.
[user]
\tname = {app.slug}[bot]
\temail = {app.bot_user_id}+{app.slug}[bot]@users.noreply.github.com
"""
        for org in scopes:
