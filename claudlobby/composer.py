...
# Per-org git credential routing. See
# documentation/plans/2026-07-27-per-org-git-credential-routing.md
...
"""
    ...
    if app:
        ...
        targets = (
            [f"https://github.com/{org}" for org in sorted(app.orgs)]
            if app.orgs
            else ["https://github.com"]
        )
        for target in targets:
            out += f"""
# GitHub App routing ({target}): cache amortizes the ~1h installation-token
# mint across git operations; the helper is referenced by absolute path so
# nothing needs to be on PATH. On hard failure it emits quit=1 (loud, never
# a fall-through to another identity).
[credential "{target}"]
\thelper = cache --timeout=3000
\thelper = {app_helper}
"""
        if app.slug and app.bot_user_id:
            out += f"""
...
"""
            ...
            out += """
# App tokens are HTTPS-only: an ssh remote would silently bypass the helper.
...
"""
    ...
    if gh_bin and not (app and not app.orgs):
        out += f"""
        ...
        if bot.github_app:
            for var_name, desc in (
                ...
                (
                    ...
                    "GitHub App installation ID (git-auth routing)",
                ),
                (
                    "GITHUB_APP_PRIVATE_KEY_PATH",
                    "Path to the App private-key .pem (0600; git-auth routing)",
                ),
            ):
                if var_name not in vars:
                    vars[var_name] = EnvVar(
