-- 0011: the RECEIVER's own delivery fact joins the transmission vocabulary
-- (chunk P, #1501) -- `received`. Delivery used to be the SENDER's inference
-- (`pane_submitted` = the input box looked empty after Enter), which was wrong
-- for 94 of 180 large sends the week before chunk O and wrong BY CONSTRUCTION:
-- the sender cannot see what the receiver ingested. A UserPromptSubmit hook in
-- the receiving session now records the byte length and sha256 of the prompt
-- it actually got (`received_bytes` / `received_sha256`, in `detail`), keyed to
-- the sender's msg_id, and the delivery JOIN (queries.DELIVERY_STATUS_SQL)
-- compares that proof to the communication's body hash. `received` is a
-- tmux/pane fact (the pane-only-tmux clause below gains it beside
-- pane_submitted/carrier_queued), and DELIBERATELY absent from every activation
-- and submission set (queries.ACTIVATION_TX_EVENTS / _TX_OPEN) so this rebuild
-- reshapes NO existing status or attention derivation -- it only makes a new
-- token insertable.
--
-- The events table CHECK enumerates the transmission tokens, and SQLite cannot
-- ALTER a CHECK, so this is the same documented 12-step rebuild migration 0010
-- ran one migration ago -- a new table with the widened list, the rows copied,
-- the old one dropped, every index recreated -- and it inherits 0010's hardened
-- concurrent-migrator handling wholesale.
--
...
DROP TABLE events;
ALTER TABLE events_m11 RENAME TO events;
