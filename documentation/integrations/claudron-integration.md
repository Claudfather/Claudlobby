...
Hooks **fail open**: any error (unresolvable vault, missing git, a stalled network) emits nothing on stdout, logs to `.claudron/hooks.log`, and exits 0 — a hook never breaks a session. The `10s` SessionEnd budget bounds **each git op** in the `sync` call, not just the network push; the write-lock *wait* itself is **not** bounded by it (`flock` blocks with no timeout), so a contended SessionEnd can run past 10s — still fail-open and lossless (see contention below). Claude Code's own hook timeout is the outer bound.
