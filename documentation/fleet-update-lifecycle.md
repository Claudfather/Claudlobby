...
| **Composed permissions** (`settings.local.json`) | **not re-read during a turn — see below** | **NO (partial: enforcement still UNMEASURED)** | yes | `claudlobby generate` |
...
### `settings.local.json`: ENFORCEMENT is UNMEASURED, and why that is not a guess

The obvious probe is inert on this estate, so no result was obtained. Recording the
...
`settings.local.json` change as **restart-gated for reads**. Setting a new key fleet-wide does not
change a running bot's behaviour. That was measured before wiring `outputStyle`, precisely
because a wrong assumption there would have made the feature silently inert.
