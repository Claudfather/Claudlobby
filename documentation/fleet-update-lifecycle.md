...
| **Composed permissions** (`settings.local.json`) | **UNMEASURED — see below** | **UNMEASURED** | yes | `claudlobby generate` |
...
### `settings.local.json` is UNMEASURED, and why that is not a guess
...
So the honest state is **UNMEASURED**, not "restart-gated". The old table asserted
the latter, and that is exactly the confidently-wrong cell this rewrite exists to
remove.
