...
| **`settings.local.json` — not re-read in-session** | `strace -f -e trace=openat` on two long-running bots (~20 h uptime) across a real turn: **0** reads of `settings*.json` by either session process. Positive controls fired in both (transcript writes, hook reads, skill reads); the 3 settings opens per trace were short-lived child processes walking the cascade | **restart-gated for reads.** Enforcement is a separate, still-open question |
...
`settings.local.json` change as **restart-gated**. Setting a new key fleet-wide does not
