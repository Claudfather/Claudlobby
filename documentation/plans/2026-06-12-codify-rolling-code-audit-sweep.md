...
**Rationale for (a):** This is the fleet's established dispatch path (manager→worker), reusing the running bot's identity, MCP servers, and credentials. **Mandatory:** the dispatch string must be prefixed `set +H;` (history-expansion lesson) and passed through `sanitize_tmux_input` (`lib-common.sh:240`). (b) spawns a context-less session that must re-establish auth/MCP and has no observability home. **Ratifier:** human.
...
|---|---|---|
| **Create** | `lib/code-audit-sweep.sh` | No-LLM selector: resolve repos → `gh` staleness query → pick stalest (deterministic tiebreak) → emit `audit_selected` event → `tmux send-keys` dispatch to owner. Sources `lib-common.sh`. |
| **Create** | `lib/install-code-audit-sweep-systemd.sh` | Linux enrollment — copy generated `.service`/`.timer` → `~/.config/systemd/user/`, `enable --now`. Verbatim copy of `install-fleet-pulse-systemd.sh:24-55`, `NAME="$SERVICE_PREFIX.code-audit-sweep"`. |
...
- [ ] **Dispatch** (F6 lean): build the audit command, prefix `set +H;`, run through `sanitize_tmux_input`, `tmux send-keys` into the owner session; follow with a bare `Enter` (dispatch lesson). Audit type rotates over `SWEEP_AUDIT_TYPES` (default `tech-debt`).
- [ ] **Skill** — write `library/skills/code-audit-sweep/SKILL.md`: pull `main`, run `/claudna:<type> --auto --output github`, **ensure issues carry the `auto-audit` label**, emit an `audit_completed` event (closes the observability loop). Zero placeholder tokens; use `{{BOT_NAME}}`/`{{CLAUDLOBBY_ROOT}}` Jinja placeholders only.
...
**Files:** Create `lib/install-code-audit-sweep-systemd.sh`, `lib/install-code-audit-sweep.sh`.

- [ ] **Linux** — copy `install-fleet-pulse-systemd.sh` verbatim; swap `NAME="$SERVICE_PREFIX.code-audit-sweep"`; keep the "run `claudlobby generate` first" guard.
...
- **`auto-audit` label provenance** — labeling currently originates in the clauDNA `/tech-debt` skill, not claudlobby (57 labeled issues confirmed live). Phase 1's skill must **guarantee** the label so the loop never silently breaks; verify during implementation.
