...
**Outstanding: Phase 6 (sunset)**, explicitly deferred by the implementing commit's own message ("coordinate via ari... overlaps alex's concurrent skill-removal work"). Re-checked 2026-08-28 — three items remain, one has since been resolved (item 3, struck below):
1. Delete `library/skills/sweep/` and `library/skills/sweep-personal/`. **Still open** — both directories still exist on disk.
2. Remove `sweep` from `fleet.yaml.example`'s skill list. **Still open** — now at `fleet.yaml.example:128` (line drifted from the `:96` originally cited, content unchanged): `skills: [dispatch, fleet-pulse, fleet-status, lifecycle, prs, status, selfcheck, sweep, restart]`.
3. ~~Update `documentation/advanced-patterns.md`'s hand-rolled `audit-tracker.json` description (still documents the old cron/Python-tracker pattern in full, ~lines 668-830).~~ **Done — this claim is now stale.** `documentation/advanced-patterns.md` §7 "Automated Code Audits" (lines 225-256) was rewritten 2026-07-06 (commit `5b51a0aa`) to document the *shipped* `fleet.sweep:` feature in full — config example, selector behavior, installers, event vocabulary — with the old `next-audit-target.py`/`audit-tracker.json` pattern reduced to a single historical parenthetical ("replaced precisely because a local tracker drifts"). Nothing left to update here.
4. Update `documentation/runbooks/pi-setup-guide.md` to add the sweep installer step (also a Phase 7 gap — never touched for this feature). **Still open** — reconfirmed 2026-08-28: zero mentions of `code-audit-sweep` or `install-code-audit-sweep` anywhere in that file (it references only the older, unrelated `bot-sweep-cron.sh`).

Per-phase status markers with evidence are inline below. `status: partially-completed` (frontmatter) reflects the split: Phase 6 is a load-bearing "no shims" requirement, not a nice-to-have, so this is deliberately not `status: completed`. Still correct as of 2026-08-28 — 3 of the original 4 sunset/doc items remain open (see above).
...
**Status: PENDING** — explicitly deferred in the implementing commit's own message ("coordinate via ari... overlaps alex's concurrent skill-removal work"). Reconfirmed 2026-08-28: still on disk: `library/skills/sweep/SKILL.md` and `library/skills/sweep-personal/SKILL.md`; `fleet.yaml.example:128` (line drifted from `:96`) still lists `sweep` in a bot's `skills:` array. **One item resolved since this was written:** `documentation/advanced-patterns.md` no longer documents the old hand-rolled `audit-tracker.json` pattern — its code-audit section was rewritten 2026-07-06 to document the shipped feature, with the old pattern reduced to one historical sentence; see "Implementation Status" near the top for the corrected outstanding checklist.
