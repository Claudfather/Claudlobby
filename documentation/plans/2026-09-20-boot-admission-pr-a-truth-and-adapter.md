...
- [ ] **Step 1:** `plugin_ensure <plugin> <claude_bin> <log>` in `lib-common.sh`: install when `installed_plugins.json` lacks the plugin (unchanged); otherwise update only when `BOOT_PLUGIN_UPDATE_ONCE` is `1` and no stamp `$CLAUDLOBBY_ROOT/state/boot/plugins-updated.<boot_epoch>.<plugin>` exists — one stamp per (boot epoch, plugin), never one stamp for the whole boot, so a second plugin's update is never skipped by the first plugin's stamp — writing the stamp under a shared lock at `$CLAUDLOBBY_ROOT/state/boot/plugins.lock`. When `resolve_boot_epoch` fails, or the stamp directory itself is unwritable, `plugin_ensure` falls back to updating on every call and logs which case it hit (`boot epoch unresolvable` / `state dir unwritable: <dir>`). `start-bot.sh:268-277` calls it per plugin.
