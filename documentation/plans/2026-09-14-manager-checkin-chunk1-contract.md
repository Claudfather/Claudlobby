...
|---|---|---|
| `planning.initiative`; `lib/checkin-propose.sh`; `dispatch-task.sh --work-item` (looking its id up, as `--supersedes` does); the proposals projection; the `propose` action; the proposal cap enforced by the door | **later — a feature build** (F1; #1559) | the empty-backlog branch of a fleet whose framework repos carry thousands of open issues |
...
| `lib/sprint-selection-record.py` (Phase 0 of #974: the batch selection record; its record/verify half has no caller today and no CLAUDE.md row) | **later — a feature build** (#1560) | it stays the sprint's instrument and the check-in record is the manager's — the two are not merged (cycle-8 precedent) |
...
| `sprint` action; `focus_*` fields | **later — feature builds** (schema 2; #1560, #1561) | not read by this chunk's skill |
...
| the `BOT_NAME` residue: `keepalive.sh:102`'s heartbeat subject and `plane_armed --require-bot` (`lib-common.sh:510-516`) key on `BOT_NAME` where every other alias uses `BOT_ID` (cycle-3 R3, gap) | **#1563** (PR 2's plan measured that the trigger does not need it, so it left the series) | the trigger runs under the timer env, and the heartbeat subject is the presence join's key — moving it needs the trigger's harness to prove the join still holds; today `start-bot.sh` exports both names, so it is latent. Chunk 2's prior art, to consume or retire by name: `lib/sprint-trigger.sh` (the shipped schedule-driven idle-manager nudge, with its busy-skip gate) and `lib/briefing-trigger.sh` (a composed per-(bot,slot) timer firing a slash command) |
| a check-in surface on the operator plane (no `view.py` route serves `system` events today; `/api/search` is comms-FTS only) | **later — a feature build** (#1562) | the outcome join gives it something to render; until then the CLI is the inspect door |
