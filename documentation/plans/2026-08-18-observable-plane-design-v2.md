...
5. Declaration rows had no stated physical shape post-F16-v2 — now: subject pair names the vault/host, `detail={vault_rev}`, and (superseding this appendix entry's original wording, round-7) `event` is REQUIRED: `revision_seen` | `scan_completed`.
