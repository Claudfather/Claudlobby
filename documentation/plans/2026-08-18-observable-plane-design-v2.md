...
3. **Exact DDL for every Lane-B family** — mechanical consequence of F16 + the model sections. *Why DDL is versioned from day one (this is not migration engineering):* the numbered files are just CREATE TABLEs replayed in order by a ~30-line `user_version` runner — but the design ships in phases to a LIVE estate (Phase 1's kernel records canary rows before Phase 2b's tables exist), and the estate runs mixed checkout versions (root pulls / update-siblings move hosts at different times), so "which phase creates which tables" and "a stale checkout must refuse a newer db" both need the version number. Fresh installs just replay 0001..N as the schema. No autogen, no down-migrations, no data transforms. Split by phase: kernel families (ingest ledger, identity, intents, attempts, work items, task attempts/events) = **migration 0001**, owned by the Phase-1 plan; all remaining observed families (`registry_snapshots`, `declaration_observed`, `presence_health_events`, `lifecycle_events`, `workstream_contracts`/`workstream_events`) = **migration 0002**, owned by the Phase-2b plan (§18) — entity payload FIELDS are final in §9b.
