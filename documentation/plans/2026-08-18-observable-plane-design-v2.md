...
**2026-08-28 docs-audit addendum:** Phase 1 (semantic kernel) and Phase 2 (ingest daemon/shim + five doors dual-write) have since shipped against this spec — commits `b696603` (#1341), `df43c70` (#1345, PR-A), `c34f12d` (#1372, PR-B), `039ede4` (post-merge gauntlet fixes); see §18 for per-phase status and `CHANGELOG.md` (`[Unreleased]`) for the detailed outcome record. This spec remains WORKING for the unbuilt phases (2b, 3–6); it is not itself "completed." All 265 `tests/test_plane_*.py` tests pass as of this addendum (verified unsandboxed, 2026-08-28).
...
| declaration observed | `events` kind=`declaration` | `revision_seen` / `scan_completed`; subject = vault\|host kind+uid pair, alias optional (§9) |
...
**Transmissions** (events kind=`transmission`, append-only): `msg_id` (ref) · `attempt_no` · `carrier` (`tmux` | `telegram-tgpost` | `telegram-bridge`; typed column, CHECK'd) · state in the stream's `event` column: `send_attempted → carrier_accepted | carrier_queued | pane_submitted | failed | unknown`, plus `recipient_acknowledged`, `duplicate_suppressed` (`carrier_queued` added PR-B per §6b #7 — the TUI accepted the payload but a busy turn parked it; accepted-not-consumed is NOT activation) · detail JSON carries `destination`, `carrier_ref` (e.g. Telegram message_id), `error`. **No transmission id** — a log row's identity is its `event_id` (the old trx id was an entity-era relic, killed in the 2026-08-24 column review). Telegram API acceptance = carrier accepted, nothing more; `pane_send_verified` establishes submitted, never receipt.
...
Event vocabulary (events kind=`task` — 20 as of PR-B: the v2.1 nineteen plus `supplied_id_not_open` per §6b #6, the join-anomaly fact the report door already names; recount ruled 2026-08-25 — the pre-deletion tuple was 20, mis-counted as 19 from the start): `dispatch_intended, transmission_failed, dispatch_submitted, accepted, rejected, progress, blocked_waiting, returned_blocked, resumed, completed, failed, cancelled, deadline_changed, superseded, reassigned, retry_created, orphaned_by_session_loss, recovered_after_restart, expired, supplied_id_not_open`. (`contract_created` deliberately absent — the work_item/assignment row IS the creation; `blocked` split per F17; **`receiver_acknowledged` DELETED per F9 v2.1** — acknowledgement is the transmission row, and activation derives through the join, closing the dual-row crash window.)
...
**PRECEDENCE RULE (2026-08-23, ends the summary-drift class):** where any prose summary in this document disagrees with `KIND_MANIFEST`, the migration DDL, or §9d's column reference, **those three govern** — summaries are informative, and a drifted summary is an editorial defect, never a semantics change.
...
**`events`** — 20 (v2.1): `kind` NOT NULL CHECK(5) · `event` NOT NULL-per-kind (every kind now requires its token; per-kind vocabulary CHECK, NULL-safe require-AND-forbid form — F3) · `carrier` (transmission-required, others forbidden) · **promoted (F7): `attempt_no` (transmission-required) · `carrier_ref` (transmission-only, indexed — Telegram reconciliation/dedupe) · `deadline` (task-only) · `successor_id` (task-only) · `renewed_until` (workstream-only)** · `msg_id` (transmission-required) · `work_item_id` (task-required) · `assignment_id` (task-only) · `workstream_id` (workstream-required) · subject triple (system: ANCHOR-PAIR — kind+uid together or both null, alias only with the pair; declaration: kind+uid REQUIRED with kind ∈ vault|host, alias optional) · `actor_uid` · `session_uid` · `severity` (system-only, registry-stamped) · `detail` (per-kind JSON tail: transmission = destination ⚑/error/part_no/part_count; task = progress/summary/pr_url; workstream = note/next_step/disposition/plan_ref; system = data{}; declaration per token — revision_seen={vault_rev}, scan_completed={scan_id, scope, counts, complete, source_rev}) · `detail_truncated`. The kind manifest (contracts.KIND_MANIFEST) is the SSOT for required/forbidden/vocabulary per kind — contracts, DDL CHECK and the INSERT-matrix test all derive from it.
...
| events kind=`declaration` | vault or host only | N : 1 | kind+uid pair required, alias optional |
| `metric_samples` | any entity | N : 1 | subject kind+uid pair (no alias by design) |
...
- **Registry scan**: 0..N `registry_snapshots` (hash-gated) + 0..N tombstones (only on complete enumeration) + `revision_seen` declarations per newly seen rev — and **every scan ends with the mandatory `scan_completed` declaration** (complete flag; tombstone validity joins on its `scan_id`) — one `scan_id` across all.
...
5. Declaration rows had no stated physical shape post-F16-v2 — now: subject pair names the vault/host, `event` REQUIRED (`revision_seen` | `scan_completed`), detail PER TOKEN: revision_seen={vault_rev}, scan_completed={scan_id, scope, counts, complete, source_rev}.
...
Before locking implementation: cold/warm emit p50/p95 on the Pi · 20–25-bot burst · concurrent system-event + communications writers · **READ benchmarks (v2.1/round-3 F7): the four derivation queries (attention, task-status, workstream-status, submitted-not-acked reconciliation) over realistically seeded mixed history with `EXPLAIN QUERY PLAN` recorded — measurable budgets: each query p50 ≤ 50 ms at the 400-item/20-bot seed on the Pi AND no un-indexed full scan of `events`; breach either and a DEFECT INVESTIGATION opens (§19.8 — the former automatic F16-v2 flip is retired; a schema change proposed on performance grounds must independently justify itself as domain-correct)** · `busy_timeout`, transaction mode, retry budget, WAL checkpoint policy, `synchronous` mode specified · disk-full behavior · startup/import time. Dependency/capability matrix for FastAPI/Pydantic/ttyd/Tailscale/frontend: optional UI features degrade without disabling the core ledger. Thumbnail capture: one bounded backend sampler per pane, cached — never multiplied by browser count.
...
- **Phase 1 — lock the semantic kernel:** rule §19's items; ratify identity, envelope, ordering, privacy, vocabulary, communications model, task state machine, spool, schema compatibility, workstream migration. **No UI.** ✅ COMPLETE (2026-08-23, docs-audit-verified 2026-08-28) — executed task-by-task per `documentation/plans/2026-08-19-observable-plane-phase1-kernel.md`, shipped commit `b696603` (PR #1341); post-ship independent review's 13 findings fixed in `6a005e2`. See that plan's Outcome section and `CHANGELOG.md`.
- **Phase 2 — headless vertical slice:** `task contract → message intent → transport attempt → acknowledgement → progress → completed/failed/blocked/overdue/orphaned`, exposed via CLI + SQLite + one reader/service layer + doctor/status + projection rebuild. Dual-write canary beside JSONL. ✅ COMPLETE (2026-08-26/27, docs-audit-verified 2026-08-28) — split across PR-A (ingest daemon/shim/host-service units/parity, dormant, commit `df43c70` #1345) and PR-B (five doors dual-write — dispatch-task/report-back/tg-post/briefing-trigger/workstream-update — behind `PLANE_EMIT_ENABLED=1`, session identity, Pi-verified latency budgets, commit `c34f12d` #1372), plus a post-merge 8-reviewer gauntlet fix round (`039ede4`). See `documentation/plans/2026-08-24-observable-plane-phase2-ingest.md` Outcome section and `CHANGELOG.md`. Operational canary-rollout status (§4's live rollout ladder onto a real fleet) is fleet-config-scoped and not verifiable from a repo checkout alone — not claimed here either way.
...
**PHASE-1 EXECUTION RECORD (2026-08-23):** the kernel is BUILT — Tasks 0–10 executed subagent-driven from the plan (commits `c9cf5bc`…`cf3a145` on this branch), TDD throughout, every file byte-diffed against the plan. **84 plane tests green** including the full crash battery on first contact (25-writer burst loses nothing; ledger↔family 1:1 under load; both derivation fixture suites; EQP mutation test; readonly→spool→drain round trip). Bench (local Mac, informational): warm p50 1.1ms, burst 25/25 committed; **the machine gate produced its first real finding — `task-status` p50 ~355ms (structural: the correlated per-assignment `ORDER BY ingest_seq LIMIT 1` subqueries walk `idx_events_kind_seq` instead of an assignment-scoped index; index-assisted, no bare scan)** — a named Phase-2 optimization item; the Pi run (binding) is expected to fail the same query and formally reopen the F16-v2 flip evaluation exactly as designed. Item statuses: canonical-bytes spec → DELIVERED (`plane/canonical.py`, golden fixtures); DDL → DELIVERED (migration 0001 incl. workstreams); ingest choice → harness DELIVERED, **Pi numbers PENDING (operator hardware)**.
...
5. **PHASE-1 GATE: LIFTED by operator decision, 2026-08-23.** Nine external review rounds (2 structural → 9 editorial; every implementation-blocking finding closed with executed probes; final open items were prose-summary drift, resolved above + governed by the precedence rule). Entry conditions adopted from the reviewer's standing requirements: (a) the loop's probe suite ships as in-repo tests exactly as planned (matrix, nine-case reducer, crash battery, EQP mutation test); (b) both derivation fixture suites pass before any timing run; (c) the Pi benchmark protocol is mandatory — five runs per query, p50 ≤ 50 ms at the stated seed, no unindexed `events` scan, machine-checked exit, and the F16-v2 flip formally reopens on failure.
6. **Round-2 closure (2026-08-24):** all ten implementation-blocking findings amended into this spec + the Phase-1 plan; executable probes rerun on the amended artifacts; Phase 1 remains GATED until the reviewer accepts the closure table. Session-identity ruling upheld by the reviewer (resume retains the id — official docs) with the F12 refinement adopted: session_uid = transcript identity, `process_uid` distinguishes concurrent resumes, SessionStart hook derives+exports, empty platform id rejected. **Mechanics assignment (round-3): the SessionStart hook, `process_uid` minting, and their tests are Phase-2 door work — named deliverables of the Phase-2 plan, not prose** (the Phase-1 kernel ships only `derive_session_uid`). **Resume-retains-id now EMPIRICALLY confirmed** (2026-08-25): the design session's own transcript — one id, one file, 2026-08-08→08-22, across a terminal death, multiple /login cycles and daily restarts — corroborating the reviewer's docs-based finding with observed behavior.
7. **Shipped-branch review closure (2026-08-24, PR #1341).** The independent review of the SHIPPED kernel returned 4 blocking + 9 should-fix findings; all 13 verified against code and spec, all confirmed, all fixed in one post-review commit with each counterexample pinned as a regression test (`tests/test_plane_review_fixes.py`). The contract-relevant rulings this locked into the kernel: **(a)** the wire envelope carries `schema_version` (stamped at finalize, preserved verbatim through spool and row; unsupported versions are ContractViolation — loud on emit, quarantined on drain — satisfying §10's versioned-envelope + unsupported-version-quarantine clauses); **(b)** capture config distinguishes ABSENT (documented default: metadata) from BROKEN (CaptureConfigError, loud — the no-silent-switch rule applied to F23), and emit validates the RAW request before capture transforms it, then the transformed form again, so §8's authored-over-cap-REJECTS holds in metadata mode; **(c)** `TASK_STATUS_SQL` implements §8's evidence-based ladder below the task-event tiers (created_not_sent → dispatch_failed / pending_unacknowledged → open, derived through the dispatch transmission chain; a failed attempt with a retry in flight reads pending, an attempt whose own verdict is failed reads dispatch_failed); **(d)** duplicate replay verifies the INCOMING family equals the ledger's stored family and the family row's ingest_seq matches the ledger's (idempotency conflict is ContractViolation, never "duplicate" success); **(e)** spool durability tightened — write-all loop, unique O_EXCL 0600 tmp names, parent-fsynced first-use directory creation, claim-by-rename draining with dead-pid recovery, envelope shape validation with empty-batch quarantine, bounded retry history; **(f)** the migration runner refuses a RACED newer version (DowngradeError), and migration **0002** ships `idx_events_task_assignment (assignment_id, ingest_seq) WHERE kind='task'` — the Pi-measured 318ms→1.2ms task-status fix (§14 gate re-passes 4/4). The F16-v2 flip evaluation the Pi failure formally reopened is CLOSED as shape-stands: the defect was index selection inside the shape, its fix is itself a partial index, and three sibling queries on the same events table pass with ~25× margin.
8. **Decision-framework ruling (2026-08-24, operator): hardware measurements inform BUDGETS and PROCESS choices, never MODEL SHAPE.** Claudlobby is a general framework; the Pi is the *instrument* for the north star's cheap-hardware floor, not a design authority. Three categories, three rules: **(a) budgets** (50ms reads, 300ms cold emit) are product commitments derived from "runs on cheap hardware" — the floor host measures compliance, it does not set the number; **(b) process choices** (direct writer vs socket daemon, batching, WAL policy) are legitimately decided by latency measured at the floor, because they are deployment-envelope questions with no domain content; **(c) model shape** (tables, columns, semantics) is decided by domain correctness alone — a hardware miss opens a DEFECT INVESTIGATION, and any schema change proposed on performance grounds must independently justify itself as domain-correct (an index qualifies; a table collapse motivated by slow hardware would not). The former F16-v2 "flip on Pi failure" condition violated (c) and is RETIRED; in the one instance it fired, the hardware proved immaterial — the Mac failed the identical query identically, the fix was a domain-neutral index, and the shape ruling stood on domain grounds.
