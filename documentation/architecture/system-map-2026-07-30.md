...
| transcript-digest.sh | SessionEnd hook: distil transcript (36x reduction), **redact secrets before model**, tail-cap 80k, Haiku extraction on capture rubric; `skipped` fact below 6-turn gate; always emits a fact, exits 0; **dormant** unless `SESSION_DIGEST_ENABLED=1` | Claude Code SessionEnd hook | hook payload, transcript JSONL | a `session_digest` **system event on the plane** via `lib/plane-emit.sh` (#1503 — no longer a `transcript-digest-<date>.jsonl` file) | claude CLI (haiku), python3, the plane | `transcript-digest.sh` redaction + the emit_row plane event | model spend gate is per-fleet env; regex redaction is best-effort; also self-gates on `PLANE_EMIT_DISABLED` |
...
- **`bot.conf`** (composed, sourced with `set -a` at boot): identity (`BOT_ID/BOT_NAME/BOT_SERVICE/TMUX_SOCKET/BOT_LABEL/BOT_DIR/TELEGRAM_STATE_DIR`), `CLAUDLOBBY_ROOT`, `CLAUDE_CONFIG_DIR` (multi-account), `CLAUDE_FLAGS` (channels/remote-control/permission-mode — default `acceptEdits`/model/effort), headless-trim vars, `FLEET_NAME/SERVICE_PREFIX/TMUX_TMPDIR/FLEET_STATE_PATH/FLEET_ROOT/FLEET_MISSION_FILE`, Telegram vars, `MODEL_STRATEGY_*`, `OBSERVABILITY_*` (pulse 300 / reap 7d / activity-stuck 1800 / dispatch-deadline 86400 (#1481) / bridge-heal), `PROJECT_*`, `WORKSTREAM_MAX_ACTIVE=12`/`WORKSTREAM_LEASE_DAYS=14`, `SWEEP_*` (owner only), `BRIEFING_*`, ecosystem pins (`CLAUDNA_VERSION`, `CLAUDRON_VAULT_PATH`, `CLAUDOSSEUM_TENANT_ID`), plugin sync vars, per-bot `env:` passthrough (anchored-path aware), `secret_files` → `$FLEET_ROOT/`-anchored exports, `MANAGER_TMUX(+_SOCKET)`, `STARTUP_PROMPT` (`composer.py:494-860`).
- **Switches** (chunk N: a door ships ON unless it deletes data, spends money, mutates operator source, or sends outbound at scale — `claudlobby/switches.py` is the registry and `claudlobby doctor --switches` the surface). Still opt-in, armed per bot in `fleet.yaml bots.<bot>.env:`: `SESSION_DIGEST_ENABLED` (model spend), `OBSERVABILITY_BRIDGE_HEAL`, `CLAUDRON_QUERY_BEFORE`. Flipped ON: `SPINDOWN_RECEIPT_ENABLED` and the plane's own doors; per-job `enroll: true` remains for `update-siblings` and `weekly-worker-restart`.
...
- **Hooks never block**: bot-vitals and transcript-digest trap all errors to exit 0; digest failure paths still emit a fact (status `error`), or disclose a failed plane record on stderr — never silently dropped (#1503).
...
| **Transcript digests** | the plane's `session_digest` **system events** (#1503 — was `state/transcript-digests/*.jsonl`) | `transcript-digest.sh` (every exit path emits a fact: ok/skipped/error) | ai-platform monitor (external, #785 — reads the plane) | `lib/plane-emit.sh`, non-blocking |
...
- `SESSION_DIGEST_ENABLED=1` — transcript-digest.sh (else exit 0 before any work); also self-gates on `PLANE_EMIT_DISABLED=1` since the plane is the only sink (#1503). Sub-knobs `SESSION_DIGEST_{MIN_TURNS,TAIL_CHARS,MODEL,TIMEOUT}` (`LOG_DIR` retired with the file sink).
...
- `DISPATCH_OVERDUE_MAX_AGE_S` (dispatch-overdue.py:69, default 86400; ≤0 disables; must stay < reap window), `OBSERVABILITY_DISPATCH_DEADLINE` (dispatch-task.sh, default 86400s = 24h since #1481; composed into every bot.conf).
...
**D1. the plane's `session_digest` system events** (#1503 — was `$ROOT/state/transcript-digests/transcript-digest-YYYY-MM-DD.jsonl`, the last production JSONL data record outside the plane)
- Writer: lib/transcript-digest.sh (SessionEnd hook; **dormant unless `SESSION_DIGEST_ENABLED=1` per fleet**, and self-gated on `PLANE_EMIT_DISABLED`; every failure path still emits a fact — or discloses a failed record on stderr — and exits 0). Redaction of secrets happens **before** the model sees text. The sink is `lib/plane-emit.sh`; no file is written.
- **Shape**: a `kind=system` event, `event=session_digest`, subject = the bot's actor (`bot:<fleet>/<name>`), `occurred_at` = the session-end instant. The record rides the opaque `data` object (system events forbid the `session_uid` stream column, and `data` is DIAGNOSTIC — truncated over-cap at ingest, never stripped by capture mode): `{status: ok|skipped|error, session_id, session_uid, cwd, reason, turns, tool_calls, transcript_bytes, digest_chars, model, context, worked, failed, would_change, reusable[, error]}` — rubric shared verbatim with clauDNA capture. `skipped` = below `SESSION_DIGEST_MIN_TURNS` (zero model cost) ≠ `ok`-with-empty-fields. Registered `notice` in `registries.SYSTEM_EVENT_SEVERITY` (informational, never an alert; kind=system is REGISTRY-governed so no migration — F19).
- Readers: `/fleet-digest` → `/fleet-observe` (reasoning pass, monitor bot); fleet-monitoring protocol; ai-platform-monitor expertise. On-estate there is NO reader of the retired JSONL; a sibling monitor reads the plane (consume-by-contract). **The strongest existing read-into-agent-workflow loop** — digest → bounded summary → cited findings.
- Lifecycle: the plane's retention lane, not unbounded file growth (the old `state/transcript-digests/` had no reaper — that concern is retired with the file).
- Correlation: `session_uid` joins the digest to the session's other plane facts (report-back task events); `session_id` ↔ vitals `tool_call.data.session` ↔ Claude transcript path.
...
| plane `session_digest` events (#1503; was transcript-digest-*.jsonl) | transcript-digest.sh (dormant opt-in) | — | — | yes — /fleet-digest → /fleet-observe (monitor loop) |
...
3. **Monitor loop (ai-platform)** — `/fleet-digest` (assemble, coverage-first, bounded) → `/fleet-observe` (five-lens reasoning; findings must cite session_id) over the plane's `session_digest` events (#1503; was `transcript-digest-*.jsonl`) + CLI rollups (`claudlobby uptime/utilization/report-back` named at fleet-monitoring.md:100). Dormant until fleets arm SESSION_DIGEST_ENABLED.
...
| state/transcript-digests/*.jsonl | **retired** (#1503 — now `session_digest` events on the plane) | the plane's retention lane |
...
15. ~~`Unverified`: whether `state/transcript-digests/` growth is handled outside the repo~~ — **resolved by #1503**: the digest is now a `session_digest` event on the plane, under the plane's retention lane; the file (and its unbounded-growth question) is retired.
